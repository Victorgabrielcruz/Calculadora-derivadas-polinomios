...
function calcularRaiz() {
  ...
  if (isNaN(n) || isNaN(k)) {
    document.getElementById("nthRootResult").innerHTML =
      "<p>Por favor, insira valores válidos para n e k.</p>";
    return;
  }
  ...
  document.getElementById(
    "nthRootResult"
  ).innerHTML = `<p>A raiz ${n}-ésima de ${k} é ${root}.</p>`;
}
...
function findRoots(parsedPoly) {
  ...
  for (let i = -100; i < 100; i += 1) {
    const x1 = i / 10;
    const x2 = (i + 1) / 10;
    const f1 = parsedPoly.reduce(
      (acc, { coef, exp }) => acc + coef * Math.pow(x1, exp),
      0
    );
    const f2 = parsedPoly.reduce(
      (acc, { coef, exp }) => acc + coef * Math.pow(x2, exp),
      0
    );
    if (f1 * f2 <= 0) {
      roots.push({ x1, x2 });
...
function newtonRaphson(
  ...
  let x0 = (x1 + x2) / 2;
  let fx = parsedPoly.reduce(
    (acc, { coef, exp }) => acc + coef * Math.pow(x0, exp),
    0
  );
  let dfx = derivative.reduce(
    (acc, { coef, exp }) => acc + coef * Math.pow(x0, exp),
    0
  );
  ...
  while (Math.abs(fx) > tolerance && iter < maxIter) {
    ...
    x0 = x0 - fx / dfx;
    fx = parsedPoly.reduce(
      (acc, { coef, exp }) => acc + coef * Math.pow(x0, exp),
      0
    );
    dfx = derivative.reduce(
      (acc, { coef, exp }) => acc + coef * Math.pow(x0, exp),
      0
    );
    iter++;
